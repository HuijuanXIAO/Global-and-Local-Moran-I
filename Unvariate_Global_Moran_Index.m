%Matlab code for Global Unvariate Moran Index
clear all;
...
s1=s1/2;
ei=-1/(n-1);   
vari=(n*((n^2-3*n+3)*s1-n*s2+3*s0^2)-b2*((n^2-n)*s1-2*n*s2+6*s0^2))/(s0^2*(n-1)*(n-2)*(n-3))-ei^2; 
z_test=(moran-ei)/vari^(1/2); 
p_value = 2 *(1- normcdf(abs(z_test)));
